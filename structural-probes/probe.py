...
class TwoWordPSDProbe(Probe):
    ...
    def __init__(self, args):
        ...
        self.proj = nn.Parameter(data=torch.zeros(
            self.model_dim, self.probe_rank))
...
class ProjectorProbe(Probe):
    """ Projects with a probe matrix
    ...
    def __init__(self, args):
        print('Constructing ProjectorProbe')
        super(ProjectorProbe, self).__init__()
        self.args = args
        self.probe_rank = args['probe']['maximum_rank']
        self.model_dim = args['model']['hidden_dim']
        self.proj = nn.Parameter(data=torch.zeros(
    ...
    def forward(self, batch):
        """ Projects each sentence in a batch.
...
class TwoWordNonPSDProbe(Probe):
    ...
    def forward(self, batch):
        ...
        """
        batchlen, seqlen, rank = batch.size()
        batch_square = batch.unsqueeze(2).expand(
            batchlen, seqlen, seqlen, rank)
        diffs = (batch_square - batch_square.transpose(1, 2)
                 ).view(batchlen*seqlen*seqlen, rank)
        psd_transformed = torch.matmul(diffs, self.proj).view(
            batchlen*seqlen*seqlen, 1, rank)
        dists = torch.bmm(psd_transformed, diffs.view(
            batchlen*seqlen*seqlen, rank, 1))
